...
// Draft -> Live for one file. Shared by the Promote command and by ConvertToLive,
// which runs it once per file in a folder subtree. Everything happens in the single
// SaveChanges the TransactionBehavior issues, so the whole operation commits or
// none of it does. There is no rollback (decision D7) — AuditLog is the undo trail.
internal sealed class PromoteExecutor
{
    private readonly DipDbContext _db;

    public PromoteExecutor(DipDbContext db) => _db = db;

    public async Task<PromoteResultDto> ExecuteAsync(
        Guid folderFileId, bool deleteMissing, string by, CancellationToken ct)
    {
        var context = await PromoteLoader.LoadAsync(_db, folderFileId, ct);
        var now = DateTime.UtcNow;

        var draftsById = context.Drafts.ToDictionary(d => d.Id);
        var (tidp, _) = await UpsertTidpAsync(context, now, by, ct);
