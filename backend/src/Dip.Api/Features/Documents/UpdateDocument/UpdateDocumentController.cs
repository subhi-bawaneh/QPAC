...
[Route("api/documents/{id:guid}")]
[Authorize]
public sealed class UpdateDocumentController : ApiControllerBase
{
    ...
    public async Task<ActionResult<DocumentDto>> Put(
        Guid id, [FromBody] UpdateDocumentRequest body, CancellationToken ct) =>
        Ok(await Dispatcher.Send(new UpdateDocumentCommand(
            ...
            body.F05Discipline, body.F06Zone, body.F07Building, body.F08ADrawingType,
            body.F08BLevel, body.F08CSequence, body.CorporateDiscipline, body.Exchanges), ct));
}
